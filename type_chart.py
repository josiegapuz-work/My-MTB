type_chart = {
    ...
    "Bug": {
        ...
        "not_very_effective": ["Fire", "Fighting", "Flying", "Ghost", "Steel", "Fairy"],
        "immune": []
    },
    "Rock": {
...
def effectiveness(attacker_type, defender_types):
    chart = type_chart[attacker_type]
    multiplier = 1.0
    for d in defender_types:
        if d in chart["super_effective"]:
            print(f"Super Effective!")
            multiplier *= 2.0
        elif d in chart["not_very_effective"]:
            print(f"Not Very Effective!")
            multiplier *= 0.5
        elif d in chart["immune"]:
            print(f"Immune")
            multiplier *= 0.0
        else:
            print(f"Normal Effect")
    
    return multiplier


print(effectiveness("Fire", ["Ice"]))